...
export default function DoctorChangePasswordPage() {
  ...
  useEffect(() => {
    fetch("/api/auth/me", { credentials: "include" })
      .then(r => r.json())
      .then(d => {
        if (!d.success || d.data.role !== "DOCTOR") {
          router.push("/login");
        }
      })
    ...
    <div style={{
      ...
      <style suppressHydrationWarning>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .cp-logo-icon { width: 40px; height: 40px; border-radius: 10px; background: linear-gradient(135deg, #22c55e, #16a34a); display: flex; align-items: center; justify-content: center; box-shadow: 0 6px 16px rgba(34,197,94,0.35); }
        .cp-logo-text { font-size:19px; font-weight: 800; color: #1e293b; letter-spacing: -0.02em; }
        .cp-logo-accent { color: #22c55e; }
        .cp-badge { display: inline-flex; align-items: center; gap: 6px; padding: 4px 12px; border-radius: 100px; font-size:10px; font-weight: 700; letter-spacing: 0.08em; text-transform: uppercase; background: rgba(34,197,94,0.1); color: #16a34a; border: 1px solid rgba(34,197,94,0.25); margin-bottom: 12px; }
        ...
        .cp-input:focus { border-color: #22c55e; box-shadow: 0 0 0 3px rgba(34,197,94,0.15); }
        ...
        .cp-btn { width: 100%; padding: 14px; border: none; border-radius: 12px; font-size:14px; font-weight: 700; font-family: 'Inter', sans-serif; cursor: pointer; position: relative; overflow: hidden; transition: transform 0.15s, box-shadow 0.15s; display: flex; align-items: center; justify-content: center; gap: 8px; background: linear-gradient(135deg, #22c55e, #16a34a); color: #fff; box-shadow: 0 4px 18px rgba(34,197,94,0.3); margin-top: 8px; }
        .cp-btn:hover:not(:disabled) { transform: translateY(-1px); box-shadow: 0 8px 26px rgba(34,197,94,0.42); }
        ...
        .cp-footer a { color: #22c55e; text-decoration: none; font-weight: 600; }
        .cp-footer a:hover { opacity: 0.8; }
        .cp-divider { height: 1px; background: #f1f5f9; margin: 20px 0; }
      `}</style>

      <div className="cp-card">
        <Link href="/doctor/dashboard" className="cp-logo">
        ...
        <div className="cp-badge">
          <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M22 12h-4l-3 9L9 3l-3 9H2"/></svg>
          Doctor Portal
        </div>
        <h2 className="cp-heading">Change Password</h2>
        <p className="cp-sub">Update your account password. Choose a strong, unique password to keep your account secure.</p>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="cp-field">
            ...
            {newPassword && (
              <>
                <div className="cp-strength-bar">{[1,2,3,4,5].map(i => (<div key={i} className="cp-strength-seg" style={{ background: i <= strength.score ? strength.color : "#e5e7eb" }} />))}</div>
                <div className="cp-strength-label" style={{ color: strength.color }}>{strength.label}</div>
                <div className="cp-requirements">
                  {[{ label: "At least 8 characters", met: newPassword.length >= 8 },{ label: "One uppercase letter (A-Z)", met: /[A-Z]/.test(newPassword) },{ label: "One lowercase letter (a-z)", met: /[a-z]/.test(newPassword) },{ label: "One number (0-9)", met: /[0-9]/.test(newPassword) }].map(req => (<div key={req.label} className={`cp-req${req.met ? " met" : ""}`}><div className="cp-req-dot"/>{req.label}</div>))}
                </div>
              </>
            )}
        ...
        <p className="cp-footer"><Link href="/doctor/dashboard">← Back to Dashboard</Link></p>
      </div>
    </div>
  );
}
