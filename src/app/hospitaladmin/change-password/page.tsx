...
export default function HospitalAdminChangePasswordPage() {
  ...
  useEffect(() => {
    fetch("/api/auth/me", { credentials: "include" })
      .then(r => r.json())
      .then(d => {
        if (!d.success || d.data.role !== "HOSPITAL_ADMIN") {
          router.push("/login");
        }
      })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok && data.success) {
        setSuccess(true);
        setTimeout(() => router.push("/hospitaladmin/dashboard"), 1500);
      } else {
  ...
  return (
    <div style={{
      ...
      <div className="cp-card">
        <Link href="/hospitaladmin/dashboard" className="cp-logo">
        ...
        <p className="cp-footer"><Link href="/hospitaladmin/profile">← Back to Profile</Link></p>
      </div>
    </div>
  );
}
