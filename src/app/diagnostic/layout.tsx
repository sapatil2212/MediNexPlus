...
const ACCENT = "#0E898F";
const ACCENT2 = "#07595D";
const ACCENT_LIGHT = "#E6F4F4";
const ACCENT_BDR = "#B3E0E0";
...
function DiagnosticLayoutContent({ children }: { children: React.ReactNode }) {
  ...
  const go = (id: string) => {
    const base = "/diagnostic/dashboard";
    ...
    <>
      <style suppressHydrationWarning>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .dg-wrap{display:flex;height:100vh;overflow:hidden;background:#f8fafc}
        .dg-overlay{display:none;position:fixed;inset:0;background:rgba(15,23,42,0.45);z-index:45;backdrop-filter:blur(2px)}
        .dg-overlay.open{display:block}
        .dg-sb{width:224px;background:#fff;border-right:1px solid #e2e8f0;display:flex;flex-direction:column;position:fixed;left:0;top:0;bottom:0;z-index:50;box-shadow:2px 0 10px rgba(14,137,143,.06);transition:transform .25s cubic-bezier(.4,0,.2,1)}
        .dg-burger{display:none;width:36px;height:36px;border-radius:10px;background:${ACCENT_LIGHT};border:1.5px solid ${ACCENT_BDR};align-items:center;justify-content:center;cursor:pointer;flex-shrink:0}
        .dg-sb-logo{padding:18px 20px 16px;border-bottom:1px solid #f1f5f9;display:flex;align-items:center;gap:10px;min-height:64px}
        .dg-logo-ic{width:36px;height:36px;background:linear-gradient(135deg,${ACCENT},${ACCENT2});border-radius:10px;display:flex;align-items:center;justify-content:center;box-shadow:0 4px 12px rgba(14,137,143,.25);flex-shrink:0}
        .dg-logo-tx{font-size:13px;font-weight:800;color:#1e293b;letter-spacing:-.02em;line-height:1.2;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
        .dg-logo-sub{font-size:10px;color:${ACCENT};font-weight:600;margin-top:1px}
        .dg-nav{flex:1;padding:10px;overflow-y:auto}
        .dg-nav-sec{font-size:10px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:#94a3b8;padding:0 8px;margin:14px 0 5px}
        .dg-nb{display:flex;align-items:center;gap:9px;width:100%;padding:9px 10px;border-radius:10px;border:none;background:none;color:#64748b;font-size:12.5px;font-weight:500;cursor:pointer;transition:all .15s;margin-bottom:2px;text-align:left;position:relative}
        .dg-nb:hover{background:${ACCENT_LIGHT};color:#334155}
        .dg-nb.on{background:linear-gradient(135deg,${ACCENT_LIGHT},#d4ebec);color:${ACCENT2};font-weight:700}
        .dg-nb-bar{display:none;width:3px;height:22px;background:linear-gradient(180deg,${ACCENT},${ACCENT2});border-radius:0 3px 3px 0;position:absolute;left:0;top:50%;transform:translateY(-50%)}
        .dg-nb.on .dg-nb-bar{display:block}
        .dg-sb-foot{padding:12px 14px 16px;border-top:1px solid #f1f5f9}
        .dg-logout{width:100%;padding:8px;border-radius:9px;background:#fff5f5;border:1px solid #fee2e2;color:#ef4444;font-size:11.5px;font-weight:600;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;transition:all .15s}
        .dg-logout:hover{background:#fee2e2}

        .dg-main{margin-left:224px;flex:1;display:flex;flex-direction:column;height:100vh;overflow:hidden}
        .dg-topbar{height:64px;background:#fff;border-bottom:1px solid #e2e8f0;display:flex;align-items:center;justify-content:space-between;padding:0 24px;position:relative;z-index:40;flex-shrink:0;box-shadow:0 1px 6px rgba(14,137,143,.06)}
        .dg-search{display:flex;align-items:center;gap:8px;background:${ACCENT_LIGHT};border:1.5px solid ${ACCENT_BDR};border-radius:10px;padding:8px 14px;width:280px;transition:border-color .2s}
        .dg-search:focus-within{border-color:${ACCENT};box-shadow:0 0 0 3px rgba(14,137,143,.15)}
        .dg-search input{background:none;border:none;outline:none;font-size:12.5px;color:#334155;width:100%}
        .dg-search input::placeholder{color:#94a3b8}
        .dg-topbar-r{display:flex;align-items:center;gap:10px}
        .dg-profile-btn{display:flex;align-items:center;gap:8px;padding:6px 10px;border-radius:10px;background:${ACCENT_LIGHT};border:1.5px solid ${ACCENT_BDR};cursor:pointer;transition:background .15s}
        .dg-profile-btn:hover{background:#d4ebec}
        .dg-profile-av{width:30px;height:30px;border-radius:8px;background:linear-gradient(135deg,${ACCENT},${ACCENT2});display:flex;align-items:center;justify-content:center;font-size:10px;font-weight:700;color:#fff;overflow:hidden}
        .dg-profile-name{font-size:11.5px;font-weight:600;color:#1e293b}
        .dg-profile-role{font-size:9px;color:${ACCENT};font-weight:500}
        .dg-content{flex:1;padding:28px 28px 40px;overflow-y:auto}
        ...
        .dd-sc{background:#fff;border-radius:14px;padding:18px;border:1px solid #e2e8f0;display:flex;align-items:center;gap:14px;cursor:default}
        ...
        .dd-card{background:#fff;border-radius:14px;border:1px solid #e2e8f0;overflow:hidden}
        ...
        @media(max-width:900px){
          .dg-sb{transform:translateX(-100%)}
          .dg-sb.open{transform:translateX(0)}
          .dg-main{margin-left:0}
          .dg-burger{display:flex}
          .dg-search{width:160px}
        }
        @media(max-width:600px){
          .dg-topbar{padding:0 14px}
          .dg-search{width:120px}
          .dg-profile-name,.dg-profile-role{display:none}
          .dg-content{padding:16px 12px 24px}
          .dd-stats{grid-template-columns:repeat(2,1fr)}
        }
      `}</style>

      <div className="dg-wrap">
        {sidebarOpen && <div className="dg-overlay open" onClick={closeSidebar} />}
        <aside className={`dg-sb${sidebarOpen ? " open" : ""}`}>
          <div className="dg-sb-logo">
            {hospitalLogo ? (
              <img src={hospitalLogo} alt="Logo" style={{ width: "100%", maxHeight: 52, objectFit: "contain", display: "block" }} />
            ) : (
              <>
                <div className="dg-logo-ic"><Stethoscope size={18} color="#fff" /></div>
                <div style={{ overflow: "hidden" }}>
                  <div className="dg-logo-tx">{hospitalName}</div>
                  <div className="dg-logo-sub">{deptProfile?.name || "Diagnostic Dept."}</div>
                </div>
              </>
            )}
          </div>
          ...
          <nav className="dg-nav">
            {NAV_SECTIONS.map(sec => (
              <div key={sec.label}>
                <div className="dg-nav-sec">{sec.label}</div>
                {sec.items.map(item => {
        ...
        <main className="dg-main">
          <header className="dg-topbar">
            <button className="dg-burger" onClick={() => setSidebarOpen(o => !o)} aria-label="Toggle sidebar">
            ...
            <div className="dg-topbar-r">
              <NotificationBell accentColor={ACCENT} bgColor={ACCENT_LIGHT} borderColor={ACCENT_BDR} />
              <div className="dg-profile-btn" onClick={() => setProfileOpen(o => !o)} style={{ position: "relative" }}>
                <div className="dg-profile-av">
                  {user?.profilePhoto
                    ? <img src={user.profilePhoto} alt="" style={{ width: "100%", height: "100%", objectFit: "cover", borderRadius: 8 }} />
                    : (user?.name ? initials(user.name) : "DH")}
                </div>
                <div>
                  <div className="dg-profile-name">{user?.name?.split(" ")[0] || "Dept. Head"}</div>
                  <div className="dg-profile-role">Diagnostic Head</div>
                </div>
                  ...
                  <>
                    ...
                    <div style={{ position: "absolute", top: "calc(100% + 8px)", right: 0, width: 220, background: "#fff", borderRadius: 12, border: "1px solid #e2e8f0", boxShadow: "0 10px 40px rgba(0,0,0,.12)", zIndex: 70, overflow: "hidden" }}>
                      <div style={{ padding: 14, borderBottom: "1px solid #f1f5f9" }}>
                        ...
                        {deptProfile && <div style={{ fontSize: 10, color: ACCENT, marginTop: 4, fontWeight: 600 }}>{deptProfile.name} · DIAGNOSTIC</div>}
                      </div>
                      <div style={{ padding: 8 }}>
