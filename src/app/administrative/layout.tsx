    ...
    <>
      <style suppressHydrationWarning>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .ad-nb{display:flex;align-items:center;gap:9px;width:100%;padding:9px 10px;border-radius:10px;border:none;background:none;color:#64748b;font-size:12.5px;font-weight:500;cursor:pointer;transition:all .15s;margin-bottom:2px;text-align:left;position:relative}
        .ad-nb:hover{background:${ACCENT_LIGHT};color:#334155}
        .ad-nb.on{background:linear-gradient(135deg,${ACCENT_LIGHT},#dbeafe);color:${ACCENT2};font-weight:700}
        .ad-nb-bar{display:none;width:3px;height:22px;background:linear-gradient(180deg,${ACCENT},${ACCENT2});border-radius:0 3px 3px 0;position:absolute;left:0;top:50%;transform:translateY(-50%)}
        .ad-nb.on .ad-nb-bar{display:block}
        .ad-sb-foot{padding:12px 14px 16px;border-top:1px solid #f1f5f9}
        ...
        <aside className={`ad-sb${sidebarOpen ? " open" : ""}`}>
          <div className="ad-sb-logo">
            {hospitalLogo ? (
              ...
              <>
                <div className="ad-logo-ic"><Stethoscope size={18} color="#fff" /></div>
                <div style={{ overflow: "hidden" }}>
                  <div className="ad-logo-tx">{hospitalName}</div>
                  <div className="ad-logo-sub">{deptProfile?.name || "Admin Dept."}</div>
                </div>
              </>
            )}
          </div>
          ...
          <nav className="ad-nav">
            {NAV_SECTIONS.map(sec => (
              <div key={sec.label}>
                <div className="ad-nav-sec">{sec.label}</div>
                {sec.items.map(item => {
        ...
        <main className="ad-main">
          <header className="ad-topbar">
            ...
            <div className="ad-topbar-r">
              ...
              <div className="ad-profile-btn" onClick={() => setProfileOpen(o => !o)} style={{ position: "relative" }}>
                <div className="ad-profile-av">
                  {user?.profilePhoto
                    ? <img src={user.profilePhoto} alt="" style={{ width: "100%", height: "100%", objectFit: "cover", borderRadius: 8 }} />
                    : (user?.name ? initials(user.name) : "AH")}
                </div>
                <div>
                  <div className="ad-profile-name">{user?.name?.split(" ")[0] || "Dept. Head"}</div>
                  <div className="ad-profile-role">Admin Head</div>
                </div>
