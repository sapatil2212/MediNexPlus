...
export default function ParentDeptLoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError("");
    try {
      const res = await fetch("/api/auth/parentdept/login", {
        method: "POST",
      ...
      if (data.success) {
        router.push("/parentdept/dashboard");
      } else {
    ...
    <>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .pdl-spin{animation:spin .7s linear infinite}
      `}</style>
      <div style={{ minHeight: "100vh", background: "linear-gradient(135deg,#0E898F 0%,#07595D 100%)", display: "flex", alignItems: "center", justifyContent: "center", padding: "20px", fontFamily: "'Inter',sans-serif" }}>
        <div style={{ background: "#fff", borderRadius: 20, padding: "40px 36px", width: "100%", maxWidth: 420, boxShadow: "0 20px 60px rgba(0,0,0,0.2)" }}>
          <div style={{ textAlign: "center", marginBottom: 28 }}>
            <div style={{ width: 56, height: 56, background: "linear-gradient(135deg,#0E898F,#07595D)", borderRadius: 16, display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 14px" }}>
              <Building2 size={26} color="#fff" />
            </div>
            <h1 style={{ fontSize:20, fontWeight: 800, color: "#1e293b" }}>Department Head Portal</h1>
            <p style={{ fontSize:12, color: "#94a3b8", marginTop: 6 }}>Sign in to manage your department & sub-departments</p>
          </div>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <button
              type="submit"
              disabled={loading}
              style={{ width: "100%", padding: "12px", background: "linear-gradient(135deg,#0E898F,#07595D)", border: "none", borderRadius: 10, color: "#fff", fontSize:13, fontWeight: 700, cursor: loading ? "not-allowed" : "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, opacity: loading ? 0.7 : 1, fontFamily: "'Inter',sans-serif" }}
            >
              {loading && <Loader2 size={16} className="pdl-spin" />}
