...
const NAV_SECTIONS = [
  {
    label: "Management",
    items: [
      { id: "overview", label: "Overview", Icon: LayoutDashboard, route: "/parentdept/dashboard" },
      { id: "subdepts", label: "Sub-Departments", Icon: Building2, route: "/parentdept/dashboard?tab=subdepts" },
    ],
  },
  {
    label: "Pharmacy",
    items: [
      { id: "queue", label: "Prescription Queue", Icon: ClipboardList, route: "/parentdept/dashboard?tab=queue" },
      { id: "inventory", label: "Inventory", Icon: Package, route: "/parentdept/dashboard?tab=inventory" },
      { id: "purchases", label: "Purchases", Icon: ShoppingCart, route: "/parentdept/dashboard?tab=purchases" },
      { id: "billing", label: "Billing", Icon: Receipt, route: "/parentdept/dashboard?tab=billing" },
    ],
  },
  {
    label: "Analytics",
    items: [
      { id: "reports", label: "Reports", Icon: BarChart2, route: "/parentdept/dashboard?tab=reports" },
      { id: "analytics", label: "Analytics", Icon: TrendingUp, route: "/parentdept/dashboard?tab=analytics" },
    ],
  },
];
...
function getActiveId(pathname: string, tab: string | null): string {
  if (pathname.startsWith("/parentdept/dashboard")) {
    if (tab && ["subdepts", "queue", "inventory", "purchases", "billing", "reports", "analytics"].includes(tab)) return tab;
    return "overview";
  }
  return "overview";
}
    ...
    <>
      <style suppressHydrationWarning>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .hd-btn-primary{padding:8px 16px;border-radius:9px;border:none;background:#0E898F;color:#fff;font-size:10px;font-weight:700;cursor:pointer;display:flex;align-items:center;gap:6px;transition:all .15s;box-shadow:0 4px 12px rgba(14,137,143,0.25)}
        ...
        .hd-msubmit{flex:2;padding:10px;border-radius:9px;border:none;background:#0E898F;color:#fff;font-size:11px;font-weight:700;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;box-shadow:0 4px 12px rgba(14,137,143,0.25)}
        ...
        <aside className={`hd-sb${sidebarOpen ? " open" : ""}`}>
          <div className="hd-sb-logo">
            {hospitalSettings?.logo
              ...
              : (<>
                <div className="hd-logo-ic"><Building2 size={18} color="white" /></div>
                <div>
                  <div className="hd-logo-tx">{hospitalSettings?.hospitalName || deptProfile?.name || "Department"}</div>
                  <div className="hd-logo-sub">Dept. Head Dashboard</div>
                </div>
              </>)
            }
          </div>
          ...
          <nav className="hd-nav">
            ...
            <div className="hd-nav-sec">Help</div>
          ...
          <div className="hd-sb-foot">
            <div className="hd-user-chip">
              <div className="hd-av">
                {user?.profilePhoto
                  ? <img src={user.profilePhoto} alt="" style={{ width: "100%", height: "100%", objectFit: "cover", borderRadius: 9 }} />
                  : (user?.name ? initials(user.name) : "DH")}
              </div>
              <div style={{ overflow: "hidden" }}>
                <div className="hd-uname">{user?.name || "Dept. Head"}</div>
          ...
          <header className="hd-topbar">
            ...
            <div className="hd-topbar-right">
              ...
              <div
                ...
                style={{ position: "relative" }}
              >
                <div className="hd-profile-av">
                  {user?.profilePhoto
                    ? <img src={user.profilePhoto} alt="" style={{ width: "100%", height: "100%", objectFit: "cover", borderRadius: 8 }} />
                    : (user?.name ? initials(user.name) : "DH")}
                </div>
                <div>
                  <div className="hd-profile-name">{user?.name?.split(" ")[0] || "Dept. Head"}</div>
                  <div className="hd-profile-role">Dept. Head</div>
                </div>
                  ...
                  <>
                    ...
                    <div style={{
                      ...
                      <div style={{ padding: 16, borderBottom: "1px solid #f1f5f9" }}>
                        ...
                        {deptProfile && <div style={{ fontSize: 10, color: "#0E898F", marginTop: 4, fontWeight: 600 }}>{deptProfile.name} · {deptProfile.type}</div>}
                      </div>
                      <div style={{ padding: 8 }}>
                        <button
                          onClick={() => { setProfileDropdownOpen(false); logout(); }}
                          style={{ width: "100%", padding: "10px 12px", borderRadius: 8, border: "none", background: "transparent", color: "#ef4444", fontSize: 12, fontWeight: 500, cursor: "pointer", display: "flex", alignItems: "center", gap: 10 }}
